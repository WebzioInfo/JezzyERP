...
export async function POST(req: NextRequest) {
    try {
        ...
        doc.setFontSize(22);
        doc.setTextColor(...TEXT_BLACK);
        doc.text("ESTIMATE/QUOTATION", W - RIGHT_MARGIN, 22, { align: "right" });
        ...
        let infoY = y;
        infoY = drawInfoRow("Quotation No:", quotation.quotationNo, infoY);
        infoY = drawInfoRow("Date:", fmtDate(quotation.date), infoY);
        if (quotation.validUntil) {
            infoY = drawInfoRow("Valid Until:", fmtDate(quotation.validUntil), infoY);
        }
        ...
        doc.setFontSize(9);
        doc.setTextColor(...TEXT_BLACK);
        doc.text("PROSPECTIVE CLIENT", LEFT_MARGIN, addrY);
        ...
        const quo = quotation as any;
        const billing = {
            name: quo.billingName || quo.client?.name,
            address1: quo.billingAddress1 || quo.client?.address1,
            address2: quo.billingAddress2 || quo.client?.address2,
            pinCode: quo.billingPinCode || quo.client?.pinCode,
            gst: quo.billingGst || quo.client?.gst
        };

        const bFinalY = drawAddress(LEFT_MARGIN, addrY, billing);

        y = Math.max(infoY, bFinalY) + 12;
        ...
        const freightVal = quotation.freightAmount?.toNumber() || 0;
        const fTaxPercent = quotation.freightTaxPercent?.toNumber() || 0;
        ...
        if (freightVal > 0) {
            drawTotalRow(`Freight Amount ${quotation.isFreightCollect ? '(Collect)' : ''}:`, fmt(freightVal));
        }
        ...
        drawTotalRow("TOTAL ESTIMATE:", `Rs. ${fmt(rounded)}`, true);
        ...
        const safeFileName = `JEZZY_EST_${seqNum}_${clientName}.pdf`.replace(/[/\\?%*:|"<>]/g, '-');
        ...
        console.error("[QUOTATION_DOWNLOAD]", err);
