...
>(({ className, ...props }, ref) => (
    <th
        ref={ref}
        className={cn(
            "h-10 px-4 text-left align-middle font-medium text-xs text-slate-500 [&:has([role=checkbox])]:pr-0",
            className
        )}
        {...props}
    />
...
>(({ className, ...props }, ref) => (
    <td
        ref={ref}
        className={cn("p-2 px-4 align-middle [&:has([role=checkbox])]:pr-0 text-slate-700 font-medium text-sm", className)}
        {...props}
    />
))
